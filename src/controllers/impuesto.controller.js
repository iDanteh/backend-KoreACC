...
export async function createImpuestoCtrl(req, res, next) {
    try {
        ...
        if (e?.original?.constraint === 'impuesto_sin_solape')
        return res.status(409).json({ message: 'Vigencia solapada para este impuesto' });
        if (e?.name === 'SequelizeUniqueConstraintError')
        return res.status(409).json({ message: 'Conflicto de unicidad' });
...
export async function listImpuestosCtrl(req, res, next) {
    try {
        const data = await listImpuestos(req.query);
        res.json(data);
    } catch (e) {
        next(e);
    }
